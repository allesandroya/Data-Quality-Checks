...
class DataQualityChecker(tk.Tk):
    ...
    def create_gui_elements(self):
        ...
        self.create_label("Copyright By: IFRS17 Team", 10, self.highlight_color, True).grid(row=2, column=0, sticky='w') 
        self.create_label("\nSelect which check to run:", 12).grid(row=3, column=0, sticky='w')    

        # Create a frame for checkboxes with the same background color
        checkbox_frame = tk.Frame(self, bg=self.bg_color)
        checkbox_frame.grid(row=4, column=0, columnspan=2, sticky='w', padx=10, pady=5)
    ...
    def date_check(self, df, date_from_filename, filename):
        ...
        ed_above_filename_year = new_df['year2'] > year
        ed_error = new_df[new_df['year2'] > year]
        ...
        ed_error.drop(ed_error.columns[indices_to_drop], axis=1, inplace=True)
        ed_error['File Name'] = filename

        date_errors = both_below_year.sum() + both_above_filename.sum() + mask3.sum() + ed_above_filename_year.sum()
        return date_errors, ed_error
...
#         return out_date_count, out_date_errors
    ...
    def clean_and_remove_blank_columns(self, df):
    ...
    def clean_excel_file(self, input_file_path, output_file_path):
        # Load the Excel file using pyxlsb engine
        excel_file = pd.ExcelFile(input_file_path, engine='pyxlsb')

        # Create an Excel writer to save the cleaned data as .xlsx
        with pd.ExcelWriter(output_file_path, engine='xlsxwriter') as writer:
            # Loop through each sheet in the Excel file
            for sheet_name in excel_file.sheet_names:
                # Load the sheet into a DataFrame using pyxlsb engine
                df = pd.read_excel(input_file_path, sheet_name=sheet_name, engine='pyxlsb') 
                
                # Clean the data
                cleaned_df = self.clean_and_remove_blank_columns(df)
                
                # Write the cleaned data to a new .xlsx file
    ...
    def load_fs10_files(self):
        if self.missing_fs10_var.get():
            ...
            # Define pattern to find the List IF MPF file
            pattern = r'List IF MPF.*\.xlsb'
            
            # Check if there is already an xlsx file
            xlsx_path = os.path.join(self.directory_path, 'List IF MPF.xlsx')
            if not os.path.exists(xlsx_path):
                # If no xlsx file, check if there is an xlsb file and convert it
                for file_name in os.listdir(self.directory_path):
                    if re.match(pattern, file_name):
                        xlsb_path = os.path.join(self.directory_path, file_name)
                        print(f"Converting {file_name} from .xlsb to .xlsx")
                        
                        # Convert xlsb to xlsx using the cleaning function
                        self.clean_excel_file(xlsb_path, xlsx_path)
                        print(f"Conversion complete: {xlsx_path}")
                        break
            
            # Load the .xlsx file after conversion or if it already exists
            fs10_path = xlsx_path
            
            # Load the first sheet and get header
            ...
            # List of additional sheets to load
    ...
    def run_checks(self):
        ...
        if self.date_error_var.get():
            all_errors['Event Date > Report Period'] = []
...
#             cash_flow_errors = self.cash_flow_date_check(df, filename.split('_')[4])
            ...
            if self.date_error_var.get():
                date_errors, ed_error = self.date_check(df, filename.split('_')[4], filename)
                if not ed_error.empty:
                    all_errors['Event Date > Report Period'].append(ed_error)
